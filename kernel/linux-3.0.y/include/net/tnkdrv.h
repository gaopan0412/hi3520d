...
struct tnkfuncs
{
    ...
    int (*tcp_sendpages) (struct sock *s, struct page **pages, int poffset, size_t psize, int flags);
    int (*tcp_sendmsg) (struct sock *s, struct iovec *iov, int iovlen, int flags);
    int (*tcp_receive) (struct sock *sk, uint32_t *seq, long timeo,
                        int target, struct iovec *to, size_t len);
    int (*tcp_read_sock) (struct sock *sk, read_descriptor_t* desc, tnk_read_actor_t recv_actor);
