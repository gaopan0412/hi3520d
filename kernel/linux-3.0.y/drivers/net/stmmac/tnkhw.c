...
#define TNK_DMA_RX_SIZE STMMAC_ALIGN(1024)
...
static struct tnkhw_conn_stats *tnk_conn_stats;
...
static int tnkhw_tx_dma_init(void)
{
	...
	for (cindex = TNK_TTX_CINDEX_START; cindex < tnk_max_connections;
		...
		t->skbuff = kzalloc(TNK_DMA_TX_SIZE * sizeof(struct sk_buff *),
				    GFP_KERNEL);
		if (!t->skbuff) {
		...
		memset(t->desc_list, 0,
		       TNK_DMA_TX_SIZE * sizeof(struct tnk_ttx_dma_desc));
		/* Clear the Tx descriptors, construct a circular linked list */
		last = &t->desc_list[TNK_DMA_TX_SIZE - 1];
		for (i = 0; i < TNK_DMA_TX_SIZE; i++) {
...
static inline int tnkhw_tx_avail_desc(unsigned cindex)
{
	...
	return t->tail + TNK_DMA_TX_SIZE - t->head - 1;
}
...
{
	struct tnkhw_tx_dma_info *t = &tnk_tx_dma_list[cindex];
	unsigned int txsize = TNK_DMA_TX_SIZE;
