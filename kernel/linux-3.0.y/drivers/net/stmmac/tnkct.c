...
void tnk_ct_adv_wnd_update(unsigned cindex)
{
...
void tnk_ct_tx_backlog_enqueue(int cindex, struct sk_buff *skb)
{
	...
	/* If there's nothing in the Tx pipeline, wake up
	 * the tasklet here
	 */
	if (tnkhw_tx_done_test(cindex)) {
#ifdef TNK_CT_USE_WORKQUEUE
		queue_work(tnk_ct_workqueue, &tnk_ct_work);
#else
		tasklet_schedule(&ct_rx_tasklet);
...
#ifdef TNK_RX_CHANNEL_FLOW_CONTROL
void tnk_ct_rx_release(int cindex, int len)
{
	struct tnkentry *e = &tnk_entry_list[cindex];
...
void tnk_ct_shutdown(void)
{
	...
	tnk_ct_close_active_connections(1);
	tnkhw_dma_stop();
#ifdef TNK_CT_USE_WORKQUEUE
...
struct tnkentry *tnk_ct_create(struct sock *sk)
{
	struct tnkentry *e;
...
int tnk_ct_stop(struct tnkentry *e, int tx_done_timeout)
{
	...
	if (unacked_bytes) {
		CONTROL_DBG("%s: Failed to drain TTX queue for connection %d"
